...
- 历史输入包：`<local-path-removed>`，只用于追溯首次引入来源，不作为后续工程依赖。
...
3. 将 Agent 执行入口落到 `../yss-design-system/SKILL.md`。
4. 将 Agent 执行检查清单更新到 `../yss-design-system/references/design-system.md`。
5. 更新 `docs/design/README.md` 和必要的 `AGENTS.md` 入口规则。
6. 运行 `../yss-design-system/scripts/validate-frontmatter.py` 和 markdown/diff 基础检查；若当前环境安装了 `PyYAML`，再运行 skill-creator 的 `quick_validate.py`。
