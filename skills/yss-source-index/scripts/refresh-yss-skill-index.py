...
FRONTEND_DOCS = {
    "components": "the consuming project's optional YSS documentation
    "hooks": "the consuming project's optional YSS documentation
    "skills": "the consuming project's optional YSS documentation
}
...
def git_source_state() -> tuple[str, str]:
    try:
...
SOURCE_COMMIT, SOURCE_WORKTREE_STATE = git_source_state()
...
def write_backend_index(skill: str, subpaths: list[str], generated_at: str) -> None:
    ...
    lines = [
        ...
        f"Source worktree: `{SOURCE_WORKTREE_STATE}`",
...
def main() -> int:
    ...
    refresh_frontend = os.environ.get("YSS_REFRESH_FRONTEND", "true").lower() not in {"0", "false", "no"}
    if refresh_frontend:
        for skill, doc_keys in FRONTEND_SKILLS.items():
            write_frontend_docs(skill, doc_keys, generated_at)
    frontend_count = len(FRONTEND_SKILLS) if refresh_frontend else 0
    print(f"Updated {len(BACKEND_SKILLS)} backend indexes and {frontend_count} frontend doc references.")
    return 0
