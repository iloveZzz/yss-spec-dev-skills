---
name: yss-db2mybatis
description: Use when 用于从数据库元数据或 DDL 生成 YSS 风格的 Domain Gateway、PO、Repository、Convertor 和 GatewayImpl。当用户要求按表结构、DDL、metadata 或数据库连接批量生成持久层代码时调用。
---
...
python3 scripts/db2mybatis.py scaffold \
  --skill-root $YSS_SKILLS_ROOT/yss-db2mybatis \
